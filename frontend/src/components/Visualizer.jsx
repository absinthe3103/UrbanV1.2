...
const Advice = ({ text }) => {
  if (!text)
    return <p style={{ color: "#475569", fontSize: 12 }}>No advice yet.</p>;
  return (
    <div>
      {text
        .replace(/\\\[[\s\S]*?\\\]/g, "")
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      >
        <div
          style={{
            height: "100%",
            width: `${Math.min(100, parseFloat(score) || 0)}%`,
            background: c.hex,
            borderRadius: 4,
            transition: "width .5s",
          }}
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial
        ...
        transparent
        opacity={isSelected ? 0.92 : hov ? 0.82 : 0.68}
        roughness={0.4}
      />
    </mesh>
  );
...
// ── 3-D: Mohr indicator with white glow ──────────────────────────────────────
function MohrIndicator({ mx, mz, colour, fs, label, isSelected, onClick }) {
  ...
  return (
    <group position={[mx, 0.02, mz]}>
      {/* White glow halo — outermost ring */}
      <mesh rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[r * 1.05, r * 1.45, 32]} />
        <meshStandardMaterial
          color="#ffffff"
          emissive="#ffffff"
          emissiveIntensity={isSelected ? 0.7 : hov ? 0.5 : 0.3}
          transparent
          opacity={isSelected ? 0.55 : hov ? 0.4 : 0.22}
        />
      </mesh>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial
          color={c.hex}
          emissive={c.emissive}
          emissiveIntensity={isSelected ? 0.9 : hov ? 0.6 : 0.3}
          transparent
          opacity={0.9}
        />
      </mesh>
      ...
      >
        ...
        <meshStandardMaterial
          color={c.hex}
          emissive={c.emissive}
          emissiveIntensity={isSelected ? 1 : hov ? 0.7 : 0.4}
          transparent
          opacity={0.9}
        />
      </mesh>

      {/* FS label — white text with coloured outline for visibility */}
      <Text
        position={[0, 0.35, 0]}
        fontSize={0.28}
        color="#ffffff"
...
function Ground({ totalL, totalW }) {
  ...
  return (
    <mesh
      position={[totalL / 2, -0.005, totalW / 2]}
      rotation={[-Math.PI / 2, 0, 0]}
      receiveShadow
    >
...
function GridLines({ L, W, ox, oz }) {
  const lines = useMemo(() => {
    const pts = [];
    for (let x = 0; x <= Math.ceil(L); x++)
      pts.push([ox + x, 0.01, oz, ox + x, 0.01, oz + W]);
    for (let z = 0; z <= Math.ceil(W); z++)
      pts.push([ox, 0.01, oz + z, ox + L, 0.01, oz + z]);
    return pts;
  }, [L, W, ox, oz]);
  return (
    <>
...
const Panel = ({
  ...
  return (
    ...
    >
        ...
        {site ? (
          <>
            ...
            <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>
              {site.siteLength}m × {site.siteWidth}m · {site.numPillars} pillars
              · {site.buildingType}
            </div>
          </>
        ) : (
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isSel ? c.hex + "33" : "transparent",
                  color: isSel ? c.hex : c.hex + "bb",
                }}
              >
                {s.siteId}
              </button>
        ...
        {!site ? (
          <div style={{ textAlign: "center", paddingTop: 40 }}>
            ...
            <div style={{ fontSize: 13, color: "#334155" }}>
              Click a site floor or select above
            </div>
          </div>
        ) : (
          <>
            <Gauge
              score={site.ssr_score}
              colour={site.ssr_colour || "red"}
              cert={site.certification}
            />
            ...
            <Sec t="Site summary" />
            <Row
              l="Avg FS (all readings)"
              v={site.avg_fs}
              vc={
                parseFloat(site.avg_fs) >= 2
                  ? "#22c55e"
                  : parseFloat(site.avg_fs) >= 1.2
                    ? "#f59e0b"
                    : "#ef4444"
              }
              mono
            ...
            <Row
              l="Load / pillar"
              v={`${site.load_per_pillar} kN`}
              vc="#60a5fa"
              mono
            />
            <Row
              l="Footing stress"
              v={`${site.pillar_stress} kN/m²`}
              vc={
                parseFloat(site.allowable_bc) >= parseFloat(site.pillar_stress)
                  ? "#22c55e"
                  : "#f59e0b"
              }
              mono
            ...
            <Sec
              t={`Mohr-Coulomb readings (${(site.mohrReadings || []).length})`}
            />
            <div
              style={{
                display: "flex",
            ...
            >
              {(site.mohrReadings || []).map((r, i) => {
                ...
                return (
                  <div
                    key={i}
                    onClick={() => onSelMohr(isSel ? null : r)}
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: 8,
                      ...
                      borderRadius: 7,
                      cursor: "pointer",
                      background: isSel ? c.hex + "22" : "#0f172a",
                      border: `1px solid ${isSel ? c.hex : c.hex + "44"}`,
                    }}
                  >
                    <div
                      style={{
                    ...
                    <span style={{ fontSize: 10, color: c.hex }}>
                      {c.label}
                    </span>
                  </div>
                );
            ...
            {reading && (
              <>
                <Sec
                  t={`Reading — (${reading.measureX}m, ${reading.measureY}m)`}
                />
                <Row
                  l="Factor of Safety"
                  v={reading.fs}
                  vc={fsCol(reading.fs_colour || "red").hex}
                  mono
                />
      ...
      {/* Invisible full-height resize edge */}
      <div
  ...
  const onResize = (e) => {
    ...
    const onMove = (e) =>
      setPanelW(
        Math.max(
          300,
          Math.min(600, resRef.current.w + (e.clientX - resRef.current.x)),
        ),
      );
    const onUp = () => {
  ...
  return (
    ...
    >
      <Panel
        ...
        selSite={selSite}
        selMohr={selMohr}
        onSelSite={setSelSite}
        onSelMohr={setSelMohr}
        panelW={panelW}
        onResize={onResize}
      />
      ...
      >
        ...
        >
          <div
            style={{
              ...
              height: 7,
              borderRadius: "50%",
              background: syncing ? "#4dff88" : "#334155",
            }}
          />
        ...
        {!visibleSite && (
          ...
          >
            ...
            <div style={{ fontSize: 13 }}>
              Select a site from the panel to view its 3D analysis
            </div>
          </div>
        )}
        ...
        <Canvas shadows style={{ background: "#0a0f1a" }}>
          <PerspectiveCamera
            makeDefault
            position={[L * 0.9 + 5, L * 0.7 + 3, W * 0.9 + 5]}
            fov={50}
          />
          <OrbitControls
            makeDefault
          ...
          {visibleSite && (
            <group>
              ...
              <SiteSlab
                ...
                ox={0}
                oz={0}
                colour={visibleSite.ssr_colour || "red"}
                isSelected={true}
                onClick={() => {}}
              />
